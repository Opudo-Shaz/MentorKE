...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── APPROVED MATCH BANNER ── */
        .match-banner {
            background: var(--white); border: 1px solid var(--green-200);
            border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow-sm);
        }
        .match-banner-header {
            padding: 12px 20px; background: var(--green-50);
            border-bottom: 1px solid var(--green-200);
            display: flex; align-items: center; gap: 8px;
        }
        .match-banner-header h2 { font-size: 14px; font-weight: 600; color: var(--green-700); }
        .match-banner-header svg { width: 16px; height: 16px; color: var(--green-700); flex-shrink: 0; }
        .match-banner-body { padding: 16px 20px; display: flex; gap: 24px; flex-wrap: wrap; }
        .match-field { }
        .match-field-label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--gray-400); margin-bottom: 4px; }
        .match-field-val   { font-size: 14px; font-weight: 600; color: var(--gray-800); }
        ...
        .btn-cancel {
            ...
            padding: 6px 12px; background: var(--red-50); color: var(--red-700);
            border: 1px solid var(--red-200); border-radius: var(--radius-md);
            font-size: 12px; font-weight: 500; font-family: 'DM Sans', sans-serif;
            cursor: pointer; transition: background 0.15s;
        }
        .btn-cancel:hover { background: var(--red-200); }
        .btn-cancel svg { width: 13px; height: 13px; }
        .no-action { font-size: 12px; color: var(--gray-400); font-style: italic; }

        /* ── BROWSE CTA BUTTON ── */
        .btn-primary { display: inline-flex; align-items: center; gap: 7px; padding: 10px 18px; background: var(--blue-800); color: var(--white); border: none; border-radius: var(--radius-md); font-size: 14px; font-weight: 500; font-family: 'DM Sans', sans-serif; cursor: pointer; text-decoration: none; transition: background 0.15s; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<%= ctx %>/app/mentee-sessions/my-requests" class="nav-link active">
...
<div class="main">
    ...
    <div class="topbar">
        <div>
            <h1>My Requests</h1>
            <p>Track your mentorship requests and their status</p>
        </div>
        <a href="<%= ctx %>/app/mentee-dashboard/" class="btn-outline">
    ...
    <div class="content">
        ...
        <div class="match-banner">
            <div class="match-banner-header">
                ...
                <h2>You have an assigned mentor</h2>
            </div>
            <div class="match-banner-body">
                <div class="match-field">
                    <div class="match-field-label">Request ID</div>
                    <div class="match-field-val">#<%= approvedMatch.getId() %></div>
                </div>
                <div class="match-field">
                    <div class="match-field-label">Mentor ID</div>
                    <div class="match-field-val">#<%= approvedMatch.getMentorId() %></div>
                </div>
                <div class="match-field">
                    <div class="match-field-label">Specialization</div>
                    <div class="match-field-val"><%= approvedMatch.getRequestedSpecialization() != null ? approvedMatch.getRequestedSpecialization() : "—" %></div>
                </div>
                <div class="match-field">
                    <div class="match-field-label">Status</div>
                    <div style="margin-top:4px;"><span class="pill pill-approved">Approved</span></div>
                </div>
            </div>
        </div>
        <% } %>
        ...
        <div class="card">
            <div class="card-header">
                <div>
                    <h2>All requests</h2>
                    <p>Your mentorship requests and their current status</p>
                </div>
                <span class="count-badge"><%= reqCount %> request<%= reqCount != 1 ? "s" : "" %></span>
            </div>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <% for (MatchRequest matchRequest : requests) {
                           String status = matchRequest.getStatus() != null ? matchRequest.getStatus() : "PENDING";
                           String pillClass = "APPROVED".equalsIgnoreCase(status) ? "pill-approved"
                                            : "REJECTED".equalsIgnoreCase(status) ? "pill-rejected"
                                            : "pill-pending";
                    %>
                        <tr>
                            <td class="id-cell">#<%= matchRequest.getId() %></td>
                            ...
                            <td>
                                ...
                                <form method="post" action="<%= ctx %>/app/mentee-sessions/cancel-request" style="margin:0;">
            ...
            <div class="empty-state">
                ...
                <p>Browse available mentors and send your first mentorship request.</p>
                <a href="<%= ctx %>/app/mentee-sessions/browse" class="btn-primary">
